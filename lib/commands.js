...
export function parseCommand(text) {
  ...
  if (sw) {
    const arg = (sw[1] ?? "").trim();
    return { kind: "switch-workspace", arg };
  }
...
/**
 * 解析一条消息是否为审批回复。
 * 支持「同意/批准/允许/是/ok/y…」「拒绝/否/n…」，可带审批编号
 * （如「同意 #3」「拒绝3」）；非审批回复返回 null。
 * @param {string} text - trim 过的消息文本。
 * @returns {{ allow: boolean, seq: number | undefined } | null}
 */
export function parseApprovalReply(text) {
  const t = text.trim();
  const m = t.match(/^(同意|批准|允许|是|yes|y|ok|okay|拒绝|不同意|否|no|n)(?:\s*#?\s*(\d+))?$/i);
  if (m === null) return null;
  return {
    allow: /^(同意|批准|允许|是|yes|y|ok|okay)$/i.test(m[1]),
    seq: m[2] !== undefined ? Number(m[2]) : undefined,
  };
}
