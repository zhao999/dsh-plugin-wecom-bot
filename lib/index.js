...
import { formatWorkspaceList, parseApprovalReply, parseCommand, resolveTargetDir } from "./commands.js";
import {
  DEFAULT_WS_ENDPOINT,
  chunkByBytes,
  extractMessage,
...
const Config = z.object({
  ...
  /**
   * agent preset：决定 agent 的工具/提示装配。留空用部署默认
   * （web profile 为 standard，提供 bash/fs 等完整编码工具）。
   * 缺失 preset 时 agent 无工具，模型只能输出伪工具调用。
   */
  ...
  /**
   * 审批超时（毫秒）：agent 请求权限提升后等待企业微信回复的最长时间，
   * 超时按「拒绝」处理。0 = 不超时（等待用户回复，agent 任务保持挂起）。
   */
  approvalTimeoutMs: z.natural().default(0),
});
...
class UserSession {
   ...
   * @param {string} key - 会话键（per-user 的 chat 键）。
   * @param {{ chatId: string, userId: string, chattype: string }} [route]
   *   企业微信路由信息：用于把审批请求推回发起任务的用户。
   * @param {Map<string, object>} [agentRoute] - agent.session.id → route 的映射，
   *   审批 answerer 据此识别 bot 创建的 agent。
   */
  constructor(ctx, config, key, route, agentRoute) {
    this.ctx = ctx;
    this.config = config;
    this.key = key;
    /** 企业微信路由（审批推送用）。 */
    this.route = route ?? { chatId: "", userId: "", chattype: "single" };
    this.agentRoute = agentRoute ?? new Map();
  ...
  async ensureAgent() {
    ...
    this.handle = await agents.create({
      ...
      setup: async (agentCtx) => {
        ...
        // 挂载 agent preset：不挂载的话工具/提示/skill 都是空层，
        // 模型会输出 <tool_calls> 伪调用文本而不是真正执行工具。
        // 必须用运行时提供的 agentPresets 服务方法（与 API 网关
        // composeAgent 一致）：插件自带的 dsh-scope 副本与运行时
        // 不是同一实例，其 scopeOf 读不到 agent 的 scope 标记，
        // 直接 import 的 mountPreset 会误报 "unscoped context"。
        const presets = agentCtx.get("agentPresets");
        const presetId = this.config.agentPreset || presets?.defaultId;
        if (presets && presetId) {
    ...
    // 记录 agent → 企业微信路由：审批 answerer 据此把请求推回发起任务的用户。
    this.agentRoute.set(String(agent.session.id), { ...this.route });
   ...
   * 下一条任务用新目录重建 agent。
   *
   * 切换同样走 tail 串行队列：若上一个任务仍在运行，先等它完成再销毁，
   * 避免中断进行中的 agent 导致结果丢失。
   */
  switchCwd(cwd) {
  ...
  async dispose() {
    ...
    this.agentRoute.delete(String(handle.agent.session.id));
...
function apply(ctx, config) {
  ...
  /** agent.session.id → 企业微信路由：审批 answerer 据此识别 bot agent 并回推。 */
  const agentRoute = new Map();
  /** 待审批表：seq → { seq, req, route, resolve, timer }。 */
  const pendingApprovals = new Map();
  /** 审批编号递增器（推送消息里让用户用「同意 #3」精确定位）。 */
  let approvalSeq = 0;

  // ── 审批 answerer（prepend 抢先接管 bot agent 的审批请求）───────────────
  // approval/request 是 waterfall 事件；宿主层 api-proxy 的 answerer 会拦截
  // 所有审批并挂起等 Web GUI 响应，bot 任务会因此卡死。prepend 让本插件的
  // answerer 先于它执行：匹配 bot 创建的 agent 时转发企业微信并等用户回复，
  // 不匹配时调 next() 放行给 GUI。
  ctx.on("approval/request", async (req, next) => {
    const route = agentRoute.get(String(req.agent.session.id));
    if (route === undefined) return next();
    if (req.signal?.aborted === true) return "cancelled";

    const seq = (approvalSeq += 1);
    const pending = {
      seq,
      req,
      route,
    ...
    pendingApprovals.set(seq, pending);
    ...
    const lines = [
      ...
      `原因：${req.reason ?? "（无说明）"}`,
      "",
      `回复「同意 #${seq}」批准，或「拒绝 #${seq}」拒绝`,
      "（也可直接回复 同意 / 拒绝，匹配你最近一条审批）",
    ];
    const target = route.chatId || route.userId;
    client.sendTo(target, lines.join("\n")).catch(() => {
      // 审批消息推送失败：不能回填就 fail-closed，避免 agent 永久挂起。
      pending.resolve?.("unavailable");
    });

    return await new Promise((resolve) => {
      pending.resolve = (outcome) => {
        if (!pendingApprovals.delete(seq)) return;
        if (pending.timer !== null) clearTimeout(pending.timer);
        if (pending.onAbort !== undefined) {
          req.signal?.removeEventListener("abort", pending.onAbort);
        }
        resolve(outcome);
      };
      pending.onAbort = () => pending.resolve?.("cancelled");
      req.signal?.addEventListener("abort", pending.onAbort, { once: true });
      if (config.approvalTimeoutMs > 0) {
        pending.timer = setTimeout(() => pending.resolve?.("rejected"), config.approvalTimeoutMs);
      }
    });
  }, { prepend: true });
  ...
  /**
   * 尝试把一条消息解析为审批回复。
   * @returns true=已处理；"no-match"=格式是审批回复但没有匹配的待审批项
   *   （应提示用户而非当任务）；false=不是审批回复。
   */
  function tryResolveApproval(text, msg) {
    if (pendingApprovals.size === 0) return false;
    const parsed = parseApprovalReply(text);
    if (parsed === null) return false;
    let target = null;
    if (parsed.seq !== undefined) {
      target = pendingApprovals.get(parsed.seq) ?? null;
    } else {
      // 无编号：匹配该用户最近一条待审批
      for (const pending of [...pendingApprovals.values()].reverse()) {
        if (pending.route.userId === msg.userid) {
          target = pending;
          break;
        }
      }
    ...
    target.resolve?.(parsed.allow ? "allowed-once" : "rejected");
    return true;
  }
  ...
  async function handleMessage(body, reqId) {
    ...
    // ── 审批回复优先 ──────────────────────────────────────────────────────
    const approvalResult = tryResolveApproval(taskText, msg);
    if (approvalResult === true) {
      client.respond(reqId, streamId, "✅ 已处理审批", true).catch(() => {});
      return;
    }
    if (approvalResult === "no-match") {
      // 格式是审批回复但没有匹配的待审批项（编号不存在/已取消/已超时）：
      // 明确提示，避免被误当成任务下发给 agent。
      client.respond(reqId, streamId, "⚠️ 当前没有匹配的待审批项（可能已处理、已取消或超时）。可发「查看工作区」查看状态。", true).catch(() => {});
      return;
    }
    ...
    try {
      ...
      if (config.sessionMode === "per-user") {
        ...
        const ephemeral = new UserSession(ctx, config, `msg-${randomUUID()}`, routeOf(msg), agentRoute);
        try {
  ...
  function sessionFor(msg) {
    ...
    if (!session) {
      session = new UserSession(ctx, config, key, routeOf(msg), agentRoute);
